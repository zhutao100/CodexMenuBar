...
`AppDelegate` owns automatic sleep-prevention synchronization. Keep the `ProcessInfo` activity active only when the preference is enabled, `codexd` is connected, and at least one runtime has an in-progress turn. Refresh it after connection, runtime, turn, and preference changes; paused, interrupted, failed, completed, disconnected, and terminating states must release the activity immediately.
