...
- `PrivacyInfo.xcprivacy` declares `UserDefaults` reason `CA92.1` for app-only Settings preferences and declares no tracking or collected data.
...
4. Confirm the archived app contains `PrivacyInfo.xcprivacy` and that its declarations still match runtime behavior.
5. Run notarization packaging outside the agent build path and keep credentials out of the repo.
